...
$(document).ready(function(){
	...
	$.getJSON("js/docgen.json",function(data){
		...
		// retrieving data from json and displaying the data
		$.each(data, function(k,v){
			...
			$.each(v,function(key,value){
				...
				$('.mobile_menu_list #'+k+'_sub').append('<li id="' + HTMLEncode(value.MethodKey) + '">' + HTMLEncode(value.Key) + '</li>');
		...
		$(".submenu li").click(function(){
			var id = this.id;
			$('html,body').animate({
				scrollTop: $('#' + id + '_content').offset().top - topOffset
			},'fast');
		});
